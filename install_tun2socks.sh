...
# 
# 用法:
#   1) 为 Alice Networks 纯 IPv6 VPS 添加 IPv4 出口: sudo ./install_tun2socks.sh -i alice
#   2) 更改 Alice 模式的 Socks5 端口: sudo ./install_tun2socks.sh -s
#   3) 卸载 tun2socks: sudo ./install_tun2socks.sh -r
#
...
#================================================================================
VERSION="1.1.3"
SCRIPT_URL="https://raw.githubusercontent.com/iBenzene/tun2socks/main/install_tun2socks.sh"
...
test_dns64_server() {
    ...
    if ping6 -c 3 -W 2 "$dns_server" &>/dev/null; then
        ...
        warning "DNS64 服务器 $dns_server 不可达。"
        return 1
    fi
}
...
test_github_access() {
    ...
    if curl -s -m 10 https://github.com >/dev/null; then
        success "GitHub 访问测试成功。"
        return 0
    else
        warning "GitHub 访问测试失败。"
        return 1
    fi
}
...
set_dns64_servers() {
    ...
    step "设置 DNS64 服务器 (用于下载 tun2socks)..."
    # 高危操作: 覆盖系统 DNS 配置, 临时修改为 DNS64 服务器
    cat >"$resolv_conf" <<EOF
...
EOF
            ...
            if test_github_access; then
                success "使用备选 DNS64 服务器 $dns_server 成功访问 GitHub。"
                return 0
            fi
        fi
    done

    error "所有 DNS64 服务器测试失败, 无法访问 GitHub。"
...
check_for_updates() {
    ...
    step "正在下载新版本..."
    TEMP_FILE="/tmp/install_tun2socks.sh.new"
...
get_custom_server_config() {
    ...
    while true; do
        read -r -p "请输入 Socks5 服务器地址 (例如: 2001:db8::1 或 1.2.3.4): " address
        if [ -n "$address" ]; then
            break
        else
            error "服务器地址不能为空。" >&2
    ...
    while true; do
        read -r -p "请输入 Socks5 服务器端口 (例如: 1080): " port
...
cleanup_ip_rules() {
    ...
    ip rule del to 100.64.0.0/10 lookup main pref 16 2>/dev/null || true
...
ExecStartPost=/sbin/ip rule add to 100.64.0.0/10 lookup main pref 16
...
ExecStop=/sbin/ip rule del to 100.64.0.0/10 lookup main pref 16
...
EOF
    ...
    step "设置服务开机自启动..."
    # 高危操作: 设置服务开机自动启动, 影响系统启动流程
    systemctl enable tun2socks.service 2>/dev/null

    step "启动服务..."
    # 高危操作: 启动服务, 将立即修改系统网络路由规则
